...
class ReviewScrapper:
    ...
    def __set_result_xl(self) -> None:
        print('🔥 결과 엑셀 생성 중..')
        # 결과 디렉터리: /result
        self.__result_dir_path = os.path.join(os.getcwd(), f"result")
        if not os.path.exists(self.__result_dir_path):
            os.mkdir(self.__result_dir_path)
        cur_time = datetime.now().strftime(
            "%Y%m%d%H%M%S"
        )  # 파일명에 붙일 현재 시각
        self.__result_xl_name = f'reviews_{cur_time}.xlsx'  # 파일명 세팅 - "reviews_(년월일시분초)" 형태
    ...
    def __write_result_xl(self):
        for row_idx, review_dict in enumerate(self.__review_dict_list):
            print(
                f'🔥 엑셀 입력 중.. {row_idx+1}/{len(self.__review_dict_list)}'
            )
            # title
